...
def Subsamplerfd(data,N, M):
    data_out = data.copy()
 
    indices = np.random.randint(data['input'].shape[0], size=N)
    
    tlen = data_out['len'][indices,0]
    tinput= data_out['input'][indices,:]
    tnormal= data_out['normal'][indices,:]
    ...
    for i in range(N):
        M1=rotation_matrix_from_vectors(tnormal[i],[1,0,0])
        data_out['input'][i]=(np.matmul(M1,data_out['input'][i].T)).T
    del data_out['normal']
